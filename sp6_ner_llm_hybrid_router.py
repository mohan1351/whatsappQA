...
class NERLLMHybridRouter:
    ...
    def __init__(self, 
                 ...
                 api_key: str = None):
        """
        Initialize router with spaCy and LLM (OpenAI or Ollama)
    ...
    def _fallback_semantic_detection(self, query: str) -> str:
        ...
        semantic_patterns = [
            ...
            'love', 'miss', 'care', 'worried', 'achievement', 'success',
            ...
            'talks like', 'speaks like', 'acts like', 'behaves like', 'writes like',
            'personality', 'attitude', 'style', 'manner', 'tone', 'vibe',
            # Roles and character types - professional
            'politician', 'poet', 'teacher', 'comedian', 'scientist',
            'artist', 'musician', 'actor', 'lawyer', 'doctor',
            'philosopher', 'historian', 'storyteller', 'journalist', 'analyst',
            # Personality archetypes
            'dreamer', 'optimist', 'pessimist', 'realist', 'cynic', 'hap'
            'skeptic', 'believer', 'pragmatist', 'idealist', 'romantic',
            # Communication traits
            'sarcastic', 'formal', 'casual', 'dramatic', 'sentimental',
            'analytical', 'spontaneous', 'serious', 'witty', 'blunt',
            'diplomat', 'rebel', 'leader', 'follower', 'mentor',
            ...
            'calm', 'calmer', 'calmest', 'calmness', 'peaceful', 'serene',
            'anxious', 'anxiety', 'anxious',
            'nervous', 'nervous',
            'depressed', 'depression', 'depressing',
            'euphoric', 'euphoria', 'euphoric',
            'melancholic', 'melancholy',
            'joyful', 'joy', 'joyful',
            'cheerful', 'cheer', 'cheerful',
            'gloomy', 'gloom', 'gloomy',
            'energetic', 'energy', 'energetic',
            'lethargic', 'lethargy',
            'confident', 'confidence', 'confidence',
            'insecure', 'insecurity', 'insecure',
            'hopeful', 'hope', 'hopeful',
            'hopeless', 'hopelessness', 'hopeless',
            'content', 'contentment', 'content',
            'frustrated', 'frustration', 'frustrate',
            'elated', 'elation', 'elated',
            'devastated', 'devastation', 'devastate',
            'tense', 'tension', 'tension',
            'relaxed', 'relax', 'relaxation',
            'stressed', 'stress', 'stressful',
            'motivated', 'motivation', 'motivate',
            'unmotivated', 'demotivated',
            'optimistic', 'optimism', 'optimistic',
            'pessimistic', 'pessimism', 'pessimistic',
            'grateful', 'gratitude', 'grateful',
...
1. semantic_concept: Is there a SEMANTIC CONCEPT (topic/theme/emotion/sentiment/attribute/preference)?
   ...
   - Communication Style: "talks like a politician", "writes like a poet", "speaks like a teacher", "acts like a comedian"
   - Personality Type: "acts like an optimist", "behaves like a pessimist", "writes like a romantic", "speaks like a skeptic"
   - Moods & Emotions: "happiest person", "most cheerful messages", "excited tone", "sounds depressed", "joyful voice", "angry person"
...
- "who talks like a politician" → semantic_concept="talks like a politician", no person
- "messages that sound sarcastic" → semantic_concept="sarcastic", no person
- "who is the happiest person" → semantic_concept="happiest person" (mood-based), no person (used as trait descriptor)
- "the most excited messages" → semantic_concept="excited messages" (mood-based), no person
- "philosophical speaker" → semantic_concept="philosophical", no person
- "optimistic tone" → semantic_concept="optimistic", no person
...
- "happiest person" → semantic concept ✅ (mood/emotional state)
- "most joyful messages" → semantic concept ✅ (mood)
- "sounds depressed" → semantic concept ✅ (mood)
- "excited tone" → semantic concept ✅ (mood)
- "calm and peaceful" → semantic concept ✅ (mood)
- "talks like a politician" → semantic concept ✅ (communication style)
- "writes like a poet" → semantic concept ✅ (communication style)
- "speaks like a teacher" → semantic concept ✅ (communication style)
- "acts like a comedian" → semantic concept ✅ (personality type)
- "behaves like an optimist" → semantic concept ✅ (personality type)
- "sarcastic tone" → semantic concept ✅ (communication trait)
- "formal manner" → semantic concept ✅ (communication style)
...
"""
    ...
    def route(self, query: str) -> Dict[str, any]:
        """
        Main routing function using NER + LLM hybrid approach
        
        Logic:
        0. Detect and convert negation (if present)
        1. IF NER finds entities AND LLM finds NO semantic → ANALYTICAL
        2. ELIF LLM finds semantic AND NER finds NO entities → SEMANTIC
        3. ELIF both find something → HYBRID
        4. ELSE neither found → UNCLEAR (ask for clarification)
        ...
        """
        # Step 0: Detect and convert negation
        is_negation, converted_query = self.detect_and_convert_negation(query)
        
        # Step 1: Extract entities with NER (use converted query)
        ner_result = self.extract_entities_with_ner(converted_query)
        
        # Step 2: Detect semantic concept with LLM (use converted query)
...
if __name__ == "__main__":
    print("="*80)
    print("NER + LLM HYBRID ROUTER - Testing")
    print("="*80)
    ...
    test_queries = [
        ...
        ("who is the happiest person", "SEMANTIC"),
        ("most excited messages", "SEMANTIC"),
        ("who talks like a politician", "SEMANTIC"),
    ...
    if result['query_type'] == 'SEMANTIC':
        ...
        pandas_agent.query(result['original_query'])
